...
func TestGroqProvider_BasicMethods(t *testing.T) {
	cfg := &config.Config{Providers: []config.Provider{{Name: "groq", APIKey: "test-key"}}}
	cfgMgr := config.NewManager("")
	cfgMgr.ApplyDefaults(cfg)
	provider := NewGroqProvider(&cfg.Providers[0])
...
func TestGroqProvider_IsStreaming(t *testing.T) {
	provider := NewGroqProvider(&config.Provider{Name: "groq"})
...
func TestGroqProvider_TransformRequest(t *testing.T) {
	provider := NewGroqProvider(&config.Provider{Name: "groq"})

	// Test Anthropic to Groq (OpenAI-compatible) request transformation
	...
	var groqReq map[string]any
	err = json.Unmarshal(result, &groqReq)
	require.NoError(t, err)

	// Verify system message was moved to messages array
	assert.NotContains(t, groqReq, "system", "system field should be removed from root")
	messages, ok := groqReq["messages"].([]any)
	...
	assert.NotContains(t, groqReq, "max_tokens", "max_tokens should be converted")
	assert.Equal(t, float64(100), groqReq["max_completion_tokens"], "should have max_completion_tokens")

	// Verify tools transformation to OpenAI format
	tools, ok := groqReq["tools"].([]any)
	...
	assert.Equal(t, "auto", groqReq["tool_choice"])
}

func TestGroqProvider_Transform(t *testing.T) {
	provider := NewGroqProvider(&config.Provider{Name: "groq"})

	groqResponse := map[string]any{
		"id":      "groq-123",
	...
	groqJSON, err := json.Marshal(groqResponse)
	require.NoError(t, err)

	result, err := provider.TransformResponse(groqJSON)
	require.NoError(t, err)
	...
	assert.Equal(t, "groq-123", anthropicResp["id"])
	assert.Equal(t, "message", anthropicResp["type"])
	assert.Equal(t, "assistant", anthropicResp["role"])
	assert.Equal(t, "groq-chat", anthropicResp["model"])
...
func TestGroqProvider_ConvertStopReason(t *testing.T) {
	provider := NewGroqProvider(&config.Provider{Name: "groq"})
	...
	for _, tt := range tests {
		t.Run(tt.groqReason, func(t *testing.T) {
			result := provider.convertStopReason(tt.groqReason)
			assert.Equal(t, tt.expectedAnthropic, *result)
		})
	}
}

func TestGroqProvider_ToolCallsTransform(t *testing.T) {
	provider := NewGroqProvider(&config.Provider{Name: "groq"})
...
func TestGroqProvider_TransformStream(t *testing.T) {
	provider := NewGroqProvider(&config.Provider{Name: "groq"})
	state := &StreamState{}
	...
	assert.Contains(t, eventStr, "groq-123")
...
func TestGroqProvider_StreamingToolCalls(t *testing.T) {
	provider := NewGroqProvider(&config.Provider{Name: "groq"})
	state := &StreamState{}
...
func TestGroqProvider_ConvertUsage(t *testing.T) {
	provider := NewGroqProvider(&config.Provider{Name: "groq"})
...
func TestGroqProvider_ConvertToolCallID(t *testing.T) {
	provider := NewGroqProvider(&config.Provider{Name: "groq"})
